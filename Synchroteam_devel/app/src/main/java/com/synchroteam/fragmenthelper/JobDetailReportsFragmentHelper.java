...
import com.synchroteam.listadapters.ReportsListAdapter;
 ...
 *
 * @author Trident Solutions
 */
public class JobDetailReportsFragmentHelper implements HelperInterface, View.OnTouchListener {
     ...
     */
    private ReportsListAdapter reportsExpandableListAdapter;

    /**
     * The list item.
     */
    private ArrayList<FamiliesBean> listItem;
    ...
    private String TAG = "JobDetailReportsFragmentHelper";
     ...
     */
    public JobDetailReportsFragmentHelper(JobDetails jobDetails, int idModel,
...
//                }
    ...
    public void doOnSyncronizeNew(int groupPosition, boolean isDelete) {
        // reportsExpandableListAdapter.notifyDataSetChanged();
        arrorBtn.setTag(Boolean.valueOf(false));
        relAttachmentContainer.setTag(Boolean.valueOf(false));
        arrorBtn.setImageResource(R.drawable.arrow_down);
        ...
        new AttachReportAddDeleteAsyncTask(this,
                groupPosition, true, isDelete).execute();
//        getSharedBlock();
//        remplir();

    }

    public void doOnSyncronizeAddSB(int groupPosition, int idFamily, int iterationCount, boolean addNewSB) {
        // reportsExpandableListAdapter.notifyDataSetChanged();
        arrorBtn.setTag(Boolean.valueOf(false));
        relAttachmentContainer.setTag(Boolean.valueOf(false));
        arrorBtn.setImageResource(R.drawable.arrow_down);
        ...
        new AttachReportAddDeleteAsyncTask(this,
                groupPosition, true, iterationCount, idFamily, addNewSB).execute();
//        getSharedBlock();
//        remplir();
        ...
        for (int j = 0; j < photo_Pdas.size(); j++) {
            if (!photo_Pdas.get(j).getCommentaire().startsWith("PIC_")) {
                ...
                jobDetails.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        attachmentLinearView.addView(getView(finalJ));
                    }
                });
    ...
    public void enregistrerScan(Bitmap bm, String code, int requestCode) {
        ...
        try {
            ...
            if (requestCode == RequestCode.REQUEST_CODE_CHILDVIEW_BARCODE) {
                ...
                dataAccessOperator.updateValue("1", code,
                        tempItems.getIdItem(), idIntervention, 0,
                        tempItems.getFlReserve(), tempItems.getNomItem(), tempItems.getIteration());
...
//                rotateFileImageIfNecessary(capturedFile);
    ...
    private class ItemsAsyncTask extends AsyncTaskCoroutine<Integer, Boolean> {
        ...
        public Boolean doInBackground(Integer... params) {
            ...
            try {
                ...
                FamiliesBean famille = listItem.get(groupPosition);
    ...
    private class DeleteSignaturesAsync extends AsyncTaskCoroutine<Void, Void> {
        ...
        public Void doInBackground(Void... params) {
            ...
            for (int i = 0; i < listItem.size(); i++) {
                FamiliesBean famille = listItem.get(i);
                ArrayList<Item> vectorItem = famille.getItems();
    ...
    public void onSharedBlockSelected(SharedBlocks sharedBlocks) {
        ...
        if (sharedBlocks != null) {
            boolean isAdded = dataAccessOperator.addSharedBlock(id_user, idModel,
                    sharedBlocks.getIdBlock(), idIntervention, 0, 0, sharedBlocks.getBlockName(), 1, position);
            if (isAdded) {
                Toast.makeText(jobDetails, jobDetails.getString(R.string.txt_ack_msg), Toast.LENGTH_SHORT).show();
//                doOnSyncronize();
                doOnSyncronizeAddSB(0, sharedBlocks.getIdBlock(),
                        0, true);
    ...
    private void getSharedBlock() {
        ...
        Logger.log("TAG", "SHARED BLOCK VALUE SIZE IS===>" + listSharedBlock.size());
        setAddSBVisibility();
    }
    ...
    public void openCameraAfterPermissionGranted() {
        ...
        AttachmentDialogReport attachmentDialog = new AttachmentDialogReport(
                jobDetails, reportsJobDetailFragment,
                JobDetailReportsFragmentHelper.this);
//        attachmentDialog.prendrePhoto();
    ...
    private class AttachReportAsyncTask extends
        ...
        ArrayList<FamiliesBean> reportList = new ArrayList<>();

        public AttachReportAsyncTask(JobDetailReportsFragmentHelper
                                             reportsJobDetailFragmentHelper) {
            this.reportsJobDetailFragmentHelper = reportsJobDetailFragmentHelper;
        }

        public AttachReportAsyncTask(JobDetailReportsFragmentHelper reportsJobDetailFragmentHelper,
                                     int groupPosition, boolean refresh, boolean isDelete) {
            this.reportsJobDetailFragmentHelper = reportsJobDetailFragmentHelper;
        ...
        public AttachReportAsyncTask(JobDetailReportsFragmentHelper jobDetailReportsFragmentHelper,
                                     int groupPosition, boolean refresh,
                                     int iterationCount, int idFamily) {
        ...
        public void onPreExecute() {
            ...
            if (listItem == null) {
                ...
                listItem = new ArrayList<FamiliesBean>();
            } else {
//                listItem.clear();
            }
...
//            }
        ...
        public Boolean doInBackground(String... params) {
            ...
            listSBCatCount = dataAccessOperator.getSBCategoryCount(idIntervention);

            ArrayList<FamiliesBean> familyList = dataAccessOperator.getSBCategoryUpdated(idIntervention,
        ...
        public void onPostExecute(Boolean result) {
            ...
            reportItemCount = listSBCatCount;

            if (reportsExpandableListAdapter == null) {

                if (listItem != null) {

                    reportsExpandableListAdapter = new ReportsListAdapter(
                ...
                if (reportsExpandableListAdapter != null) {
                    reportsExpandableListAdapter.setIndexPosition(1);
                ...
                handler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        try {
                            if (reportsExpandableListAdapter != null) {
                                if (listIndex <= listItem.size() - 1)
                                    reportsExpandableListAdapter.setIndexPosition(listIndex);
                                else
                                    reportsExpandableListAdapter.setIndexPosition(listItem.size() - 1);
...
//                expandableListView.addFooterView(footerViewNew);
            ...
            Logger.log("TAG", "SHARED BLOCK" +
                    " COUNT VALUE inventoryListCount ====>" + reportItemCount);
                ...
                if (reportsExpandableListAdapter != null) {
                    ...
                    if (loadMore && isUserScrolled && firstVisibleItem +
                        ...
                        if (ctadapter >= reportItemCount) {
                            ...
                            Logger.log("TAG", "SHARED BLOCK QUERY CALL NOT NECESSARY===>" + ctadapter);
                        } else {
...
//            hideFooterViewNew();
     ...
     * The Class SaveNewJobDataAsyncTask.
     */
    private class AttachReportAddDeleteAsyncTask extends
            AsyncTaskCoroutine<String, Boolean> {
        ...
        public AttachReportAddDeleteAsyncTask(JobDetailReportsFragmentHelper
                                                      reportsJobDetailFragmentHelper) {
            this.reportsJobDetailFragmentHelper = reportsJobDetailFragmentHelper;
        }

        public AttachReportAddDeleteAsyncTask(JobDetailReportsFragmentHelper reportsJobDetailFragmentHelper,
                                              int groupPosition, boolean refresh, boolean isDelete) {
            this.reportsJobDetailFragmentHelper = reportsJobDetailFragmentHelper;
            this.groupPos = groupPosition;
            this.refresh = refresh;
        ...
        public AttachReportAddDeleteAsyncTask(JobDetailReportsFragmentHelper jobDetailReportsFragmentHelper,
                                              int groupPosition, boolean refresh,
                                              int iterationCount, int idFamily, boolean isNewSB) {
            ...
            if (groupPos == 0) {
                ...
                if(!isDelete) {
                    if (isNewSB){
                        ...
                        pos = 2; // position to first childGroup for first Group
                    }
                }
                else
                    pos = groupPos + 2;
...
//            remplir();
            ...
            if (!isDelete) {
                ...
                ArrayList<FamiliesBean> list = dataAccessOperator.getSBCategorySingleItem(idIntervention,
                        pos, idFamily, count);

                for (int i = 0; i < list.size(); i++) {

                    FamiliesBean fm = list.get(i);
                    ...
                    if (isNewSB) {
                        listItem.add(listItem.size(), fm);
                        listShownPos = listItem.size();
                        groupPos = listShownPos;
                    } else {
                        listItem.add(groupPos + 1, fm);
                    }
                    ...
                    if (updateValues) {
                        ...
                        iterationCount = fm.getIterationCount();
                        if (!isNewSB)
                            listShownPos = groupPos;
                    }

                }
            } else {
                ArrayList<FamiliesBean> listTemp = new ArrayList<>();
                listTemp = listItem;

                if (pos <= listItem.size())
                    listItem.remove(pos - 1);

                int res = pos - 1;
                Logger.log(TAG, "DELETE POS FINAL====>" + res);

                int previousId = 0;
                int iterCnt = 1;


                //todo update logic for delete
                for (int i = 0; i < listItem.size(); i++) {
                    FamiliesBean family = new FamiliesBean();
                    family = listItem.get(i);
                    int flShared = family.getIsSharedBlock();
                    if (flShared == 1) {
                        int currentId = family.getIdFamily();
                    ...
                    family.setIterationCount(iterCnt);

                    listItem.set(i, family);
                }
//                listShownPos = listItem.size();
            }
...
//            reportsExpandableListAdapter.notifyDataSetChanged();
            ...
            Logger.log(TAG,"LIST_SIZE_VALUE IS===>"+reportItemCount);
            ...
            if (reportsExpandableListAdapter != null) {
                ...
                if (refresh && groupPos < listItem.size() - 1)
                    ...
                    expandableListView.setSelectedGroup(listItem.size() - 1);
            ...
            handler.postDelayed(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        DialogUtils.dismissProgressDialog();
                        new UpdateImageAttchmentAsyncTask().execute();
                    }
...
//            getSharedBlock();
     ...
     * The Class SaveNewJobDataAsyncTask.
     */
    private class UpdateImageAttchmentAsyncTask extends
            AsyncTaskCoroutine<String, Boolean> {


        public UpdateImageAttchmentAsyncTask() {
