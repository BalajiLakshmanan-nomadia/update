...
public class InventoryListAdapterNew extends ArrayAdapter<String> {
    ...
    public InventoryListAdapterNew(Context context, int resource,
                                   ArrayList<InventoryItemBeans> inventoryArraryList,
        ...
        this.inventoryArraryList = inventoryArraryList;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        try {
            ...
            if (inventoryArraryList != null && inventoryArraryList.size() > 0) {
                ...
                holder.txtQty.setText(String.valueOf(inventoryArraryList.get(position).getNoOfPieces()));
...
//					addItem(bundle, v, index, "0");
            ...
            // show a dialog for the serializable parts.
                ...
                } else if (fromParts && list != null && list.size() > 0) {
                    SerialNumberDialog dialog = SerialNumberDialog.getInstance(idPiece,
                            items, fromParts,takeFrom);
                    dialog.show(((AppCompatActivity) context)
                            .getSupportFragmentManager(), "");
...
//        ArrayList<InventoryDialogSerialNumber> items;
        ...
        private void initializeUI() {
            ...
            if (fromParts && takeFrom) {
                ...
                adapter = new SerialNumberAdapter(getActivity(), list, idPiece, fromParts); // instead of items , list is passed
                ...
                adapter = new SerialNumberAdapter(getActivity(), items, idPiece, fromParts);
                listView.setAdapter(adapter);
            }
    ...
    public static class SerialNumberAdapter extends BaseAdapter implements
        ...
        public SerialNumberAdapter(Context context,
                                   List<InventoryDialogSerialNumber> arrayList,
                                   String idPiece, boolean fromParts) {
...
//            holder.textView.setText(data.getName());
            if (data.getStockName() != null && data.getStockName().trim().length() > 0)
                if (idUserStock.trim().equals(data.getStockName()))
                    holder.textView.setText(data.getName());
                else
...
//                        if (data.getStockName() != null)
//                            if (!data.getStockId().equals(idUserStock) && fromParts) {
//                                listSelectedSerial.add(data);
...
//                            }
                        ...
                        //v54 changes
//                        if (data.getStockName() != null)
//                            if (!data.getStockName().equals(idUserStock) && fromParts)
