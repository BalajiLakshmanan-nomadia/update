...
public class CalendarHelper {
	 ...
	 * @param sixWeeksInCalendar the six weeks in calendar
	 * @return the full weeks
	 */
	public static ArrayList<DateTimeAlongWithJobsCountBean> getFullWeeks(int month, int year,
																		 int startDayOfWeek, boolean sixWeeksInCalendar) {
		ArrayList<DateTimeAlongWithJobsCountBean> datetimeList = new ArrayList<DateTimeAlongWithJobsCountBean>();
		...
		while (weekdayOfFirstDate > 0) {
			...
			datetimeList.add(new DateTimeAlongWithJobsCountBean(dateTime, 0, 0, 0, 0));
			weekdayOfFirstDate--;
		}

		// Add dates of current month
		for (int i = 0; i < lastDateOfMonth.getDay(); i++) {
			datetimeList.add(new DateTimeAlongWithJobsCountBean(firstDateOfMonth.plusDays(i), 0, 0, 0, 0));
		}
		...
		if (lastDateOfMonth.getWeekDay() != endDayOfWeek) {
			...
			while (true) {
				...
				datetimeList.add(new DateTimeAlongWithJobsCountBean(nextDay, 0, 0, 0, 0));
				i++;
		...
		if (sixWeeksInCalendar) {
			...
			DateTime lastDateTime = datetimeList.get(size - 1).getDateTime();
			for (int i = 1; i <= numOfDays; i++) {
				DateTime nextDateTime = lastDateTime.plusDays(i);
				datetimeList.add(new DateTimeAlongWithJobsCountBean(nextDateTime, 0, 0, 0, 0));
			}
		}
	 ...
	 *
	 * @param date the date
	 * @return the date time along with jobs count bean
	 */
	public static DateTimeAlongWithJobsCountBean convertDateToDateTime(Date date) {
		...
		return new DateTimeAlongWithJobsCountBean(new DateTime(year, javaMonth + 1, day, 0, 0, 0, 0), 0, 0, 0, 0);
	}
	 ...
	 * @return the date
	 */
	public static Date convertDateTimeToDate(DateTimeAlongWithJobsCountBean dateTime) {
		int year = dateTime.getDateTime().getYear();
		int datetimeMonth = dateTime.getDateTime().getMonth();
		int day = dateTime.getDateTime().getDay();
	 ...
	 */
	public static DateTimeAlongWithJobsCountBean getDateTimeFromString(String dateString,
			String dateFormat) {
		Date date;
		try {
